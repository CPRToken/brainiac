//src/sections/components/forms/upload/file-uploader.tsx
...
import { FileDropzone } from 'src/sections/components/forms/upload/file-dropzone';
...
export const FileUploader: FC<FileUploaderProps> = ({ onClose, open, onUpload, onUploadSuccess }) => {
  ...
  const PRIVACY_STEP = 3;
    ...
    const uploadDoc = (file: File, uniqueID: string) => {
        if (uid) {
            const docsRef = ref(storage, `${uid}/documents/${uniqueID}`);

            uploadBytes(docsRef, file).then(snapshot => {
                getDownloadURL(snapshot.ref).then(url => {
                    const docRef = doc(db, "users", uid, "documents", uniqueID);
                    setDoc(docRef, {
                        downloadUrl: url,
    ...
    const handleDrop = (newFiles: File[]): void => {
        ...
        const uniqueID = uuidv4();
        const fileLink = `http://localhost:3000/${userUrl}/documents/${uniqueID}`;
        ...
        const documentRef = ref(storage, `${uid}/documents/${uniqueID}`);
