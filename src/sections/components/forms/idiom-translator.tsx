...
export const IdiomTranslator: FC = () => {
  useProtectedPage();
 ...
 const [prompt, setPrompt] = useState<string>('');
  ...
  useEffect(() => {
    ...
    if (translationLanguage && idiom) {
      let newPrompt = t(tokens.form.idiomTranslationPrompts);

      const translationLanguageText = ` ${t(translationLanguage)} `;
      const idiomText = ` ${t(idiom)} `;

      const translationidiomWords = idiomText.split(' ');
      const title = translationidiomWords.slice(0, 3).join(' ');


      newPrompt = newPrompt
        .replace('[translationLanguage]', translationLanguageText)
        .replace('[idiom]', idiomText);
    ...
    <Box sx={{ p: 2, height: 'auto', minHeight: '500px', maxWidth: '800px', margin: 'auto' }}>
      <Stack spacing={3}>
        ...
        <Typography variant="body2">
          {t(tokens.form.idiomInstructions)}
        </Typography>

        <TextField
          fullWidth
          label={t(tokens.form.pasteIdiom)}
          name="idiom"
          value={idiom}
        ...
        <TextField
          fullWidth
          label={t(tokens.form.translationIdiom)} // Translates the label
        ...
        <Box sx={{mt: 3}}>
          ...
          <div style={{textAlign: 'center', paddingTop: '20px'}}>
            <Button
              variant="contained"
              color="primary"
              onClick={() => saveDoc(grokResponse, title, t(tokens.form.idioms))}
